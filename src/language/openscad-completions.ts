...
function makeFunctionoidSuggestion(
  ...
  if (mod.referencesChildren !== null) {
    insertText += mod.referencesChildren
      ? ' ${' + ++i + ':children}'
      : ';'
  }
  return {
...
export async function buildOpenSCADCompletionItemProvider(
  ...
  for (const [n, { deployed, symlinks }] of Object.entries(
    zipArchives
  )) {
    if (n == 'fonts') {
      continue
      ...
      try {
        ...
        if (includeMatch) {
          ...
          for (const folder of [
            ...
            files =
              folderPrefix == '' ? [...Object.keys(allSymlinks)] : []
            try {
              files = [
                ...((await readDirAsArray(fs, folder)) ?? []),
                ...files
              ]
        ...
        if (statementMatch) {
          ...
          const suggestions: CompletionItem[] = [
            ...
            ...keywordSnippets.map(snippet => ({
              label: cleanupVariables(snippet).replaceAll(
                / body/g,
                ''
              ),
          ...
          suggestions.sort(
            (a, b) =>
              a.insertText.indexOf(start) -
              b.insertText.indexOf(start)
          )
          return { suggestions }
        }
        ...
        const named: [string, CompletionItem][] = [
          ...mapObject(
            ...
            (name, mod) => [
              name,
              makeFunctionoidSuggestion(name, mod)
            ],
            name => name.indexOf(word) >= 0
          )
        ]
