...
export class Model {
  ...
  init() {
    if (
      ...
      this.state.params.lastPrompt.trim() != "" &&
      this.state.params.suggestions
    ) {
      this.processSource();
    }
  }
  ...
  set lastPrompt(lastPrompt: string) {
    if (
      this.mutate((s) => {
        s.params.lastPrompt = lastPrompt;
      })
    ) {
      this.processSource();
    }
  ...
  set suggestions(suggestions: string[]) {
    if (
      this.mutate((s) => {
        s.params.suggestions = suggestions;
      })
    ) {
      this.processSource();
    }
  ...
  render({ isPreview, now }: { isPreview: boolean; now: boolean }) {
    ...
    const { source, sourcePath, features, suggestions } = this.state.params;
